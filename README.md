...
| 模块 | 文件 | 功能 |
|------|------|------|
| **Hooks** | `settings.json.hook` | 监听用户消息，匹配关键词自动触发提示 |
| **Commands** | `commands/` | `/logs-init` 初始化扫描项目服务 |
| **Skills** | `skills/` | `/logs` 查询日志、`/logs-auto` 自动查、`/logs-solve` 查日志并修复 |
| **Scanner** | `scanner.js` | 扫描项目目录，识别框架类型，提取服务信息 |
| **Config** | `config.json` | 存储框架类型、服务列表、日志路径等配置 |
